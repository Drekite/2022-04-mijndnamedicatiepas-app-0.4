...
import { SaveStringAndKey } from '../components/safeStorage/StoreMaster'

// This function is ignored in the coverage test, since it requires
// component testing to verify the output.
...
async function readDocument (typeVarda) {
  ...
  if (result.type === 'success') {
    ...
    if (checkIfRightFile(typeVarda, columns)) {
      const PARSEDVARDA = JSON.stringify(parsingVarda(vardaLines, typeVarda))
      if (typeVarda === 'variant') {
        const SECUREKEY = 'variantSecureKey'
        const KEY = 'variantKey'
        documentImported = SaveStringAndKey(PARSEDVARDA, SECUREKEY, KEY)
      } else if (typeVarda === 'coverage') {
        const SECUREKEY = 'coverageSecureKey'
